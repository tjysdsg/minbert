...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # apply mask
    scores = scores.masked_fill(attention_mask < -900, -1e9)

    # normalize the scores
    scores = F.softmax(scores, dim=-1)
    # dropout after normalization
    scores = self.dropout(scores)

    # multiply the attention scores to the value and get back V'
    # attn = [bs, num_attention_heads, seq_len, attention_head_size]
    attn = torch.matmul(scores, value)

    # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    return torch.flatten(attn.transpose(1, 2), 2)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # feed forward
    ff = self.interm_af(self.interm_dense(x))

    # another add-norm layer
    return self.add_norm(x, ff, self.out_dense, self.out_dropout, self.out_layer_norm)
